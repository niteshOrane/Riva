...
import { resetForgotPassword } from "../../../../../services/auth/auth.service";

const re = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,}$/;

const CreatePasswordForm = ({ handleSubmit, props }) => {
  const dispatch = useDispatch();
  const email = useSelector((state) => state.common?.forgetPasswordEmail);
  ...
  const sendOTP = async (e) => {
    ...
    if (parsed && Object.keys(parsed).length && parsed?.token && parsed?.fid) {
      ...
      if (passwordNew !== passwordNewConfirm) {
        return dispatch(
          showSnackbar("Both password should be same", "warning")
        );
      }
