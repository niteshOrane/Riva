    ...
    <div className="landing-two-wrapper">
      ...
      {itemsImage.length ?
        <div>
          ...
          >
            <div className="base-image-wrapper c-pointer" onClick={() => { onCategorySelect(defaultCategory.find(e => e?.name?.toLowerCase() === itemsImage.find(m => m.position === '1')?.title?.toLowerCase())?.id) }}>
            ...
            <section>
              ...
              <div className="base-image-two c-pointer " onClick={() => { onCategorySelect(defaultCategory.find(e => e?.name?.toLowerCase() === itemsImage.find(m => m.position === '2')?.title?.toLowerCase())?.id) }}>
                <img src={`http://65.0.141.49/shop/media/mageplaza/bannerslider/banner/image/${itemsImage.find(e => e.position === '2')?.image}`} alt="" />
                <div className="banner-wrapper-text">
                  <h3>{items?.[1]?.title ?? 'TEENS'}</h3>
                  <ButtonWithArrows btnClass="bg-black color-white mx-auto" text="Shop Now" />
                </div>
              </div>
              <div className="base-image-two c-pointer " onClick={() => { onCategorySelect(defaultCategory.find(e => e?.name?.toLowerCase() === itemsImage.find(m => m.position === '3')?.title?.toLowerCase())?.id) }}>
                <img src={`http://65.0.141.49/shop/media/mageplaza/bannerslider/banner/image/${itemsImage.find(e => e.position === '3')?.image}`} alt="" />
                <div className="banner-wrapper-text">
          ...
          <div className="red-banner-wrapper c-pointer" onClick={() => { onCategorySelect(defaultCategory.find(e => e?.name?.toLowerCase() === itemsImage.find(m => m.position === '1')?.title?.toLowerCase())?.id) }}>
            <section>
              <img src={`http://65.0.141.49/shop/media/mageplaza/bannerslider/banner/image/${itemsImage.find(e => e.position === '4')?.image}`} alt="" />
            </section>
          </div>
        </div>
