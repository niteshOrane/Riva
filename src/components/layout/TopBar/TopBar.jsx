...
const TopBar = () => {
  ...
  let foundStore =
    header?.find(({ country_id }) => country_id == currentLocation?.country) ||
    {};
  const [phone, setPhone] = useState(foundStore.phone || defaultNumber);
  const [languageSelected, setLanguageSelected] = useState(
    foundStore.language || defaultLanguage
  );
  const dispatch = useDispatch();

  const handleCurrencyChange = (event) => {
    foundStore =
      header.find(({ store_id }) => store_id === event.target.value) || {};
  ...
  return (
    ...
    >
      <div className={style.topBarRight}>
        ...
        <p>
          <a className="white-space-nowrap font-white" href="tel:+971 800 7482">
            &nbsp;{phone}
          </a>
        </p>
      </div>
      ...
      >
        <div className="d-flex-all-center c-pointer">
          ...
          >
            {header?.map(({ store_name, language, store_id }) => (
              <option value={store_id}>{`${store_name} - ${language}`}</option>
            ))}
          </select>
          |{languageSelected}
        </div>
